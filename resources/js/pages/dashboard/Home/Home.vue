...
const fetchKriteria = async () => {
    ...
    try {
        ...
        // Add mock data for demonstration - replace with actual API response
        kriteria.data = data.data.map(item => ({
            ...item,
            lecturer_name: item.lecturer_name || 'Dr. John Doe', // Replace with actual lecturer name from API
            submission_date: item.submission_date || new Date().toISOString().split('T')[0], // Replace with actual date
            status: item.status || 'Submitted' // Replace with actual status
        }));
